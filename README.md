...
## Новое в v8: слой Kaspi XLSM-шаблонов

В проект добавлен отдельный слой `scripts/cs_kaspi/kaspi_templates/`, который связывает внутренние категории проекта с реальными шаблонами Kaspi:

```text
config/kaspi_templates/air_fryers.yml
config/kaspi_templates/blenders.yml
config/kaspi_templates/accessories_small_kitchen.yml
config/kaspi_templates/tabletop_ovens.yml
```

Эти YAML-файлы собраны из загруженных XLSM-шаблонов Kaspi. В них сохранены:

- точные системные коды колонок;
- русские названия полей;
- обязательность поля;
- тип поля;
- разрешённые значения из листа `values`.

`build_kaspi_exports` теперь дополнительно создаёт:

```text
artifacts/exports/kaspi_template_audit.json
artifacts/exports/kaspi_template_audit.txt
artifacts/exports/kaspi_template_blocked_products.json
artifacts/exports/kaspi_template_ready_air_fryers.csv
artifacts/exports/kaspi_template_ready_blenders.csv
artifacts/exports/kaspi_template_ready_accessories_small_kitchen.csv
artifacts/exports/kaspi_template_ready_tabletop_ovens.csv
```

CSV-файлы `kaspi_template_ready_*.csv` имеют заголовки с точными кодами колонок Kaspi-шаблона. Их можно использовать как промежуточный слой для переноса в официальный XLSM-шаблон Kaspi.

Важно: `commercial_ready_products` означает, что товар продаваемый по WB и прошёл коммерческую логику. `kaspi_template_ready_products` означает, что товар дополнительно прошёл проверку обязательных полей конкретного Kaspi-шаблона. Если товар попал в `kaspi_template_blocked_products.json`, его нельзя загружать без исправления указанных полей.
