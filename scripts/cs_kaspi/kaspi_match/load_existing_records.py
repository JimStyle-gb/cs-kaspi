...
def run() -> dict[str, Any]:
    """Load existing Kaspi products from generated/API state.

    v7 WB-only does not use input/ as a manual source. Until Kaspi API sync is
    implemented, the state file may be absent and the layer safely returns no
    existing records.
    """
    state_file = path_from_config("kaspi_existing_state_file")
    ...
    return {
        "source": "kaspi_existing_state_file",
        "state_file": str(state_file.as_posix()),
        "files": [str(p.as_posix()) for p in files],
        "records": records,
        "errors": errors,
        "note": "No manual input/ files are used. Existing Kaspi products will come from API/state sync later.",
    }
