...
def write_preview_txt(path: Path, preview: dict) -> None:
    ...
    for p in preview.get("products", []):
        lines.extend([
            ...
            f"  price_rule: WB lowest exact variant -> +30% -> floor to 00",
            f"  kaspi_template: {p.get('kaspi_template_key') or '-'} / {p.get('kaspi_template_status')}",
            f"  kaspi_template_errors: {p.get('kaspi_template_errors')}",
