...
def input_dirs() -> dict[str, Path]:
    """Return official raw-cache directories.

    Historical function name is kept to avoid touching supplier fetch modules,
    but v7 WB-only does not write downloaded official pages into input/.
    Everything generated by scripts goes to artifacts/raw/.
    """
    cfg = get_config()
    dirs = cfg.get("raw_cache_dirs", {})
    return {
        "catalog_pages": ROOT / dirs.get("catalog_pages_dir", "artifacts/raw/official/demiand/catalog_pages"),
        "product_pages": ROOT / dirs.get("product_pages_dir", "artifacts/raw/official/demiand/product_pages"),
        "manuals": ROOT / dirs.get("manuals_dir", "artifacts/raw/official/demiand/manuals"),
    }
