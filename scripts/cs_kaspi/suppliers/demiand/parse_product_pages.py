...
from .utils import clean_images, extract_json_ld, get_config, make_soup, parse_price_to_number, slug_from_url, state_paths
...
def _clean_product_id(value: str | None) -> str | None:
    ...
    if not cleaned:
        return None
    cleaned = re.split(r"\bКатегори[яи]:|\bМетк[аи]:", cleaned, maxsplit=1)[0].strip()
...
def _extract_meta_value(meta_text: str, label: str) -> str | None:
    ...
    tail = re.split(r"\bКатегори[яи]:|\bМетк[аи]:", tail, maxsplit=1)[0]
    return _clean_product_id(tail)


def _extract_categories_from_meta(meta_text: str) -> list[str]:
    match = re.search(r"Категори[яи]:\s*(.+?)(?:\bМетк[аи]:|$)", meta_text)
    if not match:
        return []
    return [normalize_spaces(x) for x in match.group(1).split(",") if normalize_spaces(x)]
...
def _pick_available(soup, offers: dict[str, Any]) -> bool | None:
    ...
    stock_text = normalize_spaces(stock_tag.get_text(" ", strip=True) if stock_tag else None).lower()
...
def _price_from_page(price_tag, offers: dict[str, Any]) -> int | None:
    visible_price = parse_price_to_number(price_tag.get_text(" ", strip=True) if price_tag else None)
    if visible_price is not None:
        return visible_price
    if isinstance(offers, dict):
        return parse_price_to_number(str(offers.get("price") or ""))
    return None
...
def run(product_pages_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for page in product_pages_payload.get("pages", []):
        ...
        title = normalize_spaces(h1.get_text(" ", strip=True) if h1 else None)
        meta_tag = soup.select_one(".product_meta")
        meta_text = normalize_spaces(meta_tag.get_text(" ", strip=True) if meta_tag else None)
        ...
        short_desc = normalize_spaces(short_desc_tag.get_text(" ", strip=True) if short_desc_tag else None)
        description_tag = soup.select_one("#tab-description, .woocommerce-Tabs-panel--description")
        description_text = normalize_spaces(description_tag.get_text(" ", strip=True) if description_tag else None)
        ...
        for row in soup.select("table.woocommerce-product-attributes tr"):
            ...
            key = normalize_spaces(key_tag.get_text(" ", strip=True) if key_tag else None)
            value = normalize_spaces(value_tag.get_text(" ", strip=True) if value_tag else None)
            if key:
                attrs[key] = value
        ...
        images = clean_images(images_raw)
        ...
        breadcrumbs = [normalize_spaces(x.get_text(" ", strip=True)) for x in soup.select(".woocommerce-breadcrumb a, .woocommerce-breadcrumb span") if normalize_spaces(x.get_text(" ", strip=True))]
        categories = [normalize_spaces(x.get_text(" ", strip=True)) for x in soup.select(".product_meta .posted_in a") if normalize_spaces(x.get_text(" ", strip=True))]
        if not categories:
            categories = _extract_categories_from_meta(meta_text)
        ...
        product_id = sku_value or _extract_meta_value(meta_text, "Артикул") or _clean_product_id(product_graph.get("sku"))
        ...
        product = {
            ...
            "official": {
                ...
                "price": _price_from_page(current_price_tag, offers),
