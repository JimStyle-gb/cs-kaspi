...
from scripts.cs_kaspi.kaspi_templates.build_template_rows import build_rows as build_template_rows
from scripts.cs_kaspi.kaspi_templates.write_files import write_template_exports
from scripts.cs_kaspi.kaspi_templates.build_template_rows import build_row as build_template_row
from scripts.cs_kaspi.kaspi_templates.validate_rows import validate_row as validate_template_row
...
CSV_HEADERS = [
    ...
    "kaspi_template_key",
    "kaspi_template_status",
    "kaspi_template_errors",
    "brand",
    "model_key",
    "variant_key",
    "official_article",
...
def _template_info(product: dict[str, Any]) -> dict[str, Any]:
    try:
        built = build_template_row(product)
        template_key = built.get("template_key") or ""
        errors = list(built.get("errors") or [])
        template = built.get("template") or {}
        row = built.get("row") or {}
        if row and template:
            validation = validate_template_row(row, template)
            errors.extend(validation.get("errors") or [])
        return {
            "kaspi_template_key": template_key,
            "kaspi_template_status": "template_ready" if not errors else "template_blocked",
            "kaspi_template_errors": errors,
        }
    except Exception as exc:
        return {
            "kaspi_template_key": "",
            "kaspi_template_status": "template_check_failed",
            "kaspi_template_errors": [str(exc)],
        }
...
def _export_item(product: dict[str, Any], *, action: str) -> dict[str, Any]:
    ...
    template = _template_info(product)
    ...
    return {
        ...
        "kaspi_template_key": template.get("kaspi_template_key"),
        "kaspi_template_status": template.get("kaspi_template_status"),
        "kaspi_template_errors": template.get("kaspi_template_errors"),
    ...
    write_json(
        path,
        {
            "meta": {
                ...
                "note": "Kaspi category codes are stored in config/categories.yml and verified against data/kaspi/reference/kaspi_categories.json when available. Missing or invalid codes block live-create.",
                "categories": len(rows),
...
def _write_category_audit_txt(path: Path, plan: dict[str, Any]) -> None:
    rows = _category_audit_rows(plan)
    missing_count = sum(1 for row in rows if not row.get("kaspi_category_code"))
    lines = [
        "CS-Kaspi Kaspi category audit",
        "note: category_code берётся из config/categories.yml и сверяется с data/kaspi/reference/kaspi_categories.json",
        f"categories_missing_code: {missing_count}",
        "",
    ]
    if not rows:
...
def _write_category_todo_txt(path: Path, plan: dict[str, Any]) -> None:
    ...
    if not rows:
        ...
        lines.append("Все ready-категории сейчас имеют category_code.")
...
def run() -> dict[str, Any]:
    ...
    template_data = build_template_rows(products)
    template_meta = template_data.setdefault("meta", {})
    template_meta.setdefault("built_at", plan.get("meta", {}).get("built_at"))
    template_files = write_template_exports(template_data)
    ...
    write_json(path_from_config("artifacts_exports_dir") / "kaspi_export_summary.json", meta)
