...
_ALLOWED_FILL_SOURCES = {"ozon", "wb", "manual"}
_FORBIDDEN_FILL_SOURCES = {"google", "kaspi"}
_MATCH_CONFIRM_COLUMNS = ("match_article_ok", "match_model_ok", "match_color_ok", "match_specs_ok")
...
def _is_true_value(value: str) -> bool:
    return _text(value).lower() in {"true", "1", "yes", "да", "y", "ok", "совпадает", "available", "in_stock"}


def _is_unknown_value(value: str) -> bool:
    return _text(value).lower() in {"", "-", "?", "unknown", "не знаю", "неизвестно"}
...
def _normalized_source(value: str) -> str:
    ...
    if source in {"wildberries", "wb"}:
        return "wb"
    if source in {"ozon", "озон"}:
        return "ozon"
    if source in {"manual", "ручной", "manually"}:
        return "manual"
    if source in {"google", "гугл"}:
        return "google"
    if source in {"kaspi", "каспи"}:
        return "kaspi"
    return source
...
def _source(row: dict[str, Any]) -> str:
    source = _normalized_source(_fill_first(row, "fill_source", "source", "market_source"))
...
def _row_problems(row: dict[str, Any], out: dict[str, str], file: str, row_index: int) -> list[dict[str, Any]]:
    ...
    source = _normalized_source(out.get("source", ""))
    if source in _FORBIDDEN_FILL_SOURCES:
        add("google/kaspi cannot be used as market price source; use only ozon/wb/manual")
    elif source not in _ALLOWED_FILL_SOURCES:
        add("unsupported fill_source/source; allowed: ozon, wb, manual")

    if any(key in row for key in _MATCH_CONFIRM_COLUMNS):
        for key in _MATCH_CONFIRM_COLUMNS:
            value = _first(row, key)
            if _is_unknown_value(value):
                add(f"filled worklist row must confirm {key}=true")
            elif not _is_true_value(value):
                add(f"filled worklist row has {key} not confirmed")
...
def run() -> dict[str, Any]:
    ...
    report = {
        ...
        "problems": problems,
         "note": "Fill only fill_* market columns in generated worklists. Context/search columns are ignored and do not activate rows. Empty rows are allowed. Partially filled rows fail fast. Google/Kaspi cannot be used as market price sources. Generated worklists require match_* confirmations.",
    }
