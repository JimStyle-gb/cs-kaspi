...
""")
    ...
    for idx, row in enumerate(rows, start=1):
        ...
        fill_html = "<br>".join([
            ...
            f'article/model/color/specs ok: <span class="fill">{escape(_text(row.get("match_article_ok")))}/{escape(_text(row.get("match_model_ok")))}/{escape(_text(row.get("match_color_ok")))}/{escape(_text(row.get("match_specs_ok")))}</span>',
            f'comment: <span class="fill">{escape(_short(row.get("match_comment"), 80))}</span>',
        ])
...
3. Ozon/WB — источники цены и наличия. Google/Kaspi — только ручная проверка похожих карточек и конкурентов.
4. Для нужных товаров заполни fill_source, fill_url, fill_price, fill_available, fill_stock, fill_lead_time_days.
5. Обязательно подтверди совпадение товара: match_article_ok, match_model_ok, match_color_ok, match_specs_ok.
6. Заполненный CSV можно положить в input/market/worklists/ — importer сам создаст стандартный input.
7. Либо перенеси заполненные строки в боевой input-файл: input/market/manual/demiand_manual_market_real.csv, input/market/ozon/demiand_ozon_market.csv или input/market/wb/demiand_wb_market.csv.
8. product_key менять нельзя.
