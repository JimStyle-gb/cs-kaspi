...
_URL_REQUIRED_SOURCES = {"ozon", "wb", "manual"}
_ALLOWED_PRICE_SOURCES = {"ozon", "wb", "manual"}
_FORBIDDEN_PRICE_SOURCES = {"google", "kaspi"}
...
def _validate_market_state(market_state: dict[str, Any]) -> dict[str, Any]:
    ...
    for product_key, market in products.items():
        ...
        if source in _FORBIDDEN_PRICE_SOURCES:
            problems.append(_problem("critical", "Google/Kaspi cannot activate product or set market price", product_key=product_key, source=source))
        elif source not in _ALLOWED_PRICE_SOURCES:
            problems.append(_problem("critical", "unsupported market price source; allowed: ozon, wb, manual", product_key=product_key, source=source))

        if source in _URL_REQUIRED_SOURCES and not market.get("market_url"):
            problems.append(_problem("critical", "sellable market product is missing market_url", product_key=product_key, source=source))
    ...
    return {
        ...
        "problems": problems,
        "note": "Header-only market files are allowed. Active Ozon/WB/manual sellable rows must have price, stock, availability and market_url. Google/Kaspi are helper links only and cannot activate products.",
    }
