...
def _seed_key_values(value: Any) -> set[str]:
    if value is None:
        return set()
    if isinstance(value, (list, tuple, set)):
        return {key for key in (_text(item) for item in value) if key}
    text = _text(value)
    return {text} if text else set()


def _active_seed_keys(seeds: list[dict[str, Any]], raw_cards: list[dict[str, Any]]) -> set[str]:
    keys = {
        _text(seed.get("seed_key"))
        for seed in seeds
        if isinstance(seed, dict) and seed.get("enabled", True) is not False and _text(seed.get("seed_key"))
    }
    if keys:
        return keys
    return {
        ...
        for key in _seed_key_values(card.get("seed_key"))
    }


def _filter_previous_by_active_seed_keys(
    previous: dict[str, dict[str, Any]],
    active_seed_keys: set[str],
) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    if not active_seed_keys:
        return previous, {}
    kept: dict[str, dict[str, Any]] = {}
    ignored: dict[str, dict[str, Any]] = {}
    for wb_id, row in previous.items():
        previous_seed_keys = _seed_key_values(row.get("seed_keys"))
        # Old baselines may have no seed_keys at all. Keep those rows rather than silently losing history.
        if not previous_seed_keys or previous_seed_keys & active_seed_keys:
            kept[wb_id] = row
        else:
            ignored[wb_id] = row
    return kept, ignored
...
def _write_wb_missing_txt(
    path: Path,
    *,
    previous_count: int,
    current_count: int,
    missing_rows: list[dict[str, Any]],
    active_seed_keys: list[str] | None = None,
    previous_raw_count: int | None = None,
    ignored_previous_inactive_seed_count: int = 0,
) -> None:
    lines = [
        ...
        "Для brand_all-only baseline старые товары из отключённых seed/search источников игнорируются и не считаются пропавшими.",
        "",
        f"active_seed_keys: {_text(active_seed_keys or []) or '-'}",
        f"previous_seen_products_raw: {previous_raw_count if previous_raw_count is not None else previous_count}",
        f"ignored_previous_inactive_seed_products: {ignored_previous_inactive_seed_count}",
...
def _build_wb_seen_audit(raw_cards: list[dict[str, Any]], seeds: list[dict[str, Any]], built_at: str) -> dict[str, Any]:
    state_dir = path_from_config("artifacts_state_dir")
    state_dir.mkdir(parents=True, exist_ok=True)
    baseline_path = state_dir / "wb_seen_products_baseline.json"
    previous_raw = _read_previous_wb_seen(baseline_path)
    active_keys = _active_seed_keys(seeds, raw_cards)
    previous, ignored_previous = _filter_previous_by_active_seed_keys(previous_raw, active_keys)
    ...
    baseline = {
        "built_at": built_at,
        "active_seed_keys": sorted(active_keys),
        "note": "Autogenerated current WB sellable DEMIAND baseline. Missing audit compares only active production seeds, so old wide-search/category baselines do not create false missing noise.",
        "products_count": len(current_rows),
        "products": current_rows,
    }
    write_json(baseline_path, baseline)
    return {
        "previous_raw_count": len(previous_raw),
        "previous_count": len(previous),
        "ignored_previous_inactive_seed_count": len(ignored_previous),
        "active_seed_keys": sorted(active_keys),
...
def run(
    *,
    ...
    built_at = now_iso()
    wb_seen_audit = _build_wb_seen_audit(raw_cards, seeds, built_at)
    ...
    write_json(out_dir / "wb_missing_products_audit.json", {
        "built_at": built_at,
        "active_seed_keys": wb_seen_audit.get("active_seed_keys", []),
        "previous_seen_products_raw": wb_seen_audit.get("previous_raw_count", 0),
        "ignored_previous_inactive_seed_products": wb_seen_audit.get("ignored_previous_inactive_seed_count", 0),
    ...
    _write_wb_missing_txt(
        ...
        active_seed_keys=wb_seen_audit.get("active_seed_keys", []),
        previous_raw_count=int(wb_seen_audit.get("previous_raw_count") or 0),
        ignored_previous_inactive_seed_count=int(wb_seen_audit.get("ignored_previous_inactive_seed_count") or 0),
