...
def normalize_card(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "seed_role": raw.get("seed_role"),
        "review_only": bool(raw.get("review_only")),
        "market_id": market_id,
        "brand": _clean_text(raw.get("brand")),
        "brand_id": _int_or_none(raw.get("brand_id") or raw.get("brandId")),
        "brand_status": _clean_text(raw.get("brand_status")),
        "brand_is_expected": raw.get("brand_is_expected"),
        "title": title,
        "url": url,
        "image": raw.get("image"),
